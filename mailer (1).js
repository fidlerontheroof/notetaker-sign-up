...
function switchDenied({ to, name, fromDate, toDate }) {
  const link = checkLink(to);
  const subject = `Switch request update`;
  const text =
    `Hi ${name},\n\n` +
    `Your request to switch from ${fromDate} to ${toDate} was not approved — ` +
    `you're still signed up for ${fromDate}. If you'd like to try a different date, ` +
    `you can submit another request any time here:\n${link}`;
  const html =
    `<p>Hi ${escapeHtml(name)},</p>` +
    `<p>Your request to switch from ${escapeHtml(fromDate)} to ${escapeHtml(toDate)} was not approved — ` +
    `you're still signed up for <strong>${escapeHtml(fromDate)}</strong>. If you'd like to try a different date, ` +
    `you can submit another request any time here:<br><a href="${link}">${link}</a></p>`;
  return sendMail({ to, subject, text, html });
}
