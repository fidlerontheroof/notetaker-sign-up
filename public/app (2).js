...
function renderDatesList() {
  ...
  const note = $('dates-note');
  if (mode === 'switch') {
    note.textContent =
      "Pick any date below. If it has room, you'll be switched immediately. If it's full, you'll stay on your current date and your request goes to the instructor for approval — you'll get an email either way.";
  } else {
    note.textContent = '';
  }
  ...
  allDates.forEach((c) => {
    ...
    // In normal sign-up mode, a full date really can't be picked.
    // In switch mode, a full date CAN be picked — it just becomes a
    // pending request instead of an instant swap, so it must stay clickable.
    const isBlocked = isMine || (isFull && mode === 'signup');
    const row = document.createElement('div');
    row.className = 'date-row';
    const label = isMine ? 'Current' : isFull ? (mode === 'switch' ? 'Request' : 'Full') : 'Select';
    row.innerHTML = `
      <div class="date-main">
      ...
      <div>
        ...
        <button class="small" ${isBlocked ? 'disabled' : ''}>${label}</button>
      </div>
    `;
  ...
  try {
    if (mode === 'switch') {
      ...
      if (data.status === 'swapped') {
        ...
        $('form-success').textContent =
          `That date is currently full, so your request has been sent to the instructor for approval. ` +
          `You're still signed up for your original date for now — nothing changes unless it's approved. ` +
          `You'll get an email either way (approved or not).`;
      }
      mode = 'signup';
      await checkStatus();
      // Leave the message on screen for a few seconds instead of hiding it
      // instantly — otherwise it's set and hidden in the same tick and the
      // student never actually sees it (this was a real bug).
      setTimeout(() => { $('signup-form-card').style.display = 'none'; }, 6000);
    } else {
